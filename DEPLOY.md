...
สิทธิ์นี้คุมสามปุ่มในหน้าเหตุการณ์พร้อมกัน — **มอบหมาย · ยกระดับ · สั่งการปฏิบัติ**
ยศที่ไม่เข้าเงื่อนไขจะเห็นข้อความ "ยศของคุณไม่ตรงกับเงื่อนไข" แทนทั้งสามปุ่ม

| ได้ | ไม่ได้ |
|---|---|
| ร.ต. · ร.ท. · ร.อ.<br>น.ต. · น.ท. · น.อ.<br>พล.ร.ต. · พล.ร.ท. · พล.ร.อ. | จ.ต. · จ.ท. · จ.อ.<br>พ.จ.ต. · พ.จ.ท. · พ.จ.อ.<br>พลฯ · อื่น ๆ |

> 🔓 **ช่องโหว่ที่ยังเปิดอยู่ — ยศเป็นค่าที่ผู้ใช้เลือกเองตอนสมัคร ไม่มีใครตรวจสอบ**
>
> ใครสมัครใหม่แล้วเลือก "พล.ร.อ." จะได้สิทธิ์มอบหมายงานทันที โดยไม่ต้องให้ admin อนุมัติ
> ต่างจาก `role` ที่ trigger กันไว้แน่นหนา — **`rank` ไม่มีการป้องกันแบบเดียวกัน**
> และผู้ใช้ยังแก้ยศตัวเองผ่าน API ได้ด้วย เพราะ `profiles_guard_role` กันเฉพาะคอลัมน์ `role`
>
> ถ้าจะปิดช่องนี้ มีสองอย่างที่ต้องทำ:
> 1. เพิ่ม `rank` เข้าไปใน trigger `profiles_guard_role` (กันการแก้ยศตัวเองหลังสมัคร)
> 2. ให้ admin ยืนยันยศก่อนมีผล — เช่น เพิ่มคอลัมน์ `rank_verified boolean default false`
>    แล้วให้ `can()` เช็คคอลัมน์นี้ด้วย
>
